// Fallout 76 Interactive Map - Main Application
...
class FalloutMapApp {
    ...
    updateMapMarkers() { this.map.setVisibleCategories(this.visibleCategories); }

    savePreferences() {
        localStorage.setItem('fo76map_prefs', JSON.stringify(Array.from(this.visibleCategories)));
    }

    loadPreferences() {
        try {
            const stored = localStorage.getItem('fo76map_prefs');
            if (stored) {

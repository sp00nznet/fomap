// Fallout New Vegas Interactive Map - Main Application
...
class FalloutMapApp {
    ...
    buildMarkerList() {
        ...
        if (collectibles && collectibles.snowglobes) {
            for (const sg of collectibles.snowglobes) {
                this.allMarkers.push({
                    id: sg.id, name: sg.name, category: 'snowglobes',
                    x: sg.x, y: sg.y, description: sg.description,
                    location: sg.location, isCollectible: true
                });
            }
        }
    }
    ...
    initCollectibleToggles() {
        ...
        container.innerHTML = '';
        if (collectibles && collectibles.snowglobes) {
            const item = this.createToggleItem('snowglobes', 'Snow Globes', collectibles.snowglobes.length, true);
            container.appendChild(item);
        }
    }
    ...
    savePreferences() {
        ...
        localStorage.setItem('fnvmap_prefs', JSON.stringify(prefs));
    }

    loadPreferences() {
        try {
            const stored = localStorage.getItem('fnvmap_prefs');
            if (stored) {
